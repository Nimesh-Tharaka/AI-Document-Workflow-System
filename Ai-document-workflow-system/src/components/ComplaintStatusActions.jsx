...
function BaseStatusActions({ title, currentStatus, loading, onChangeStatus, onAddComment }) {
  const [comment, setComment] = useState("");

  async function handleStatusClick(status) { await onChangeStatus(status, comment); setComment(""); }
  async function handleCommentOnly() {
    if (!comment.trim()) { alert("Please enter a comment first."); return; }
    await onAddComment(comment); setComment("");
  }
...
export function ComplaintStatusActions(props) {
  return <BaseStatusActions title="Complaint Actions" {...props} />;
}

export function LeaveRequestStatusActions(props) {
  return <BaseStatusActions title="Leave Request Actions" {...props} />;
}

// default exports for direct imports
export default function ComplaintStatusActionsDefault(props) {
  return <BaseStatusActions title="Complaint Actions" {...props} />;
}