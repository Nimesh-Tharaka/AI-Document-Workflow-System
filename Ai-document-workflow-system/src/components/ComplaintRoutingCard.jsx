// ComplaintRoutingCard.jsx
import { useState } from "react";

export function ComplaintRoutingCard({ currentDepartment, currentRoutingStatus, currentRoutingReason, loading, onAutoRoute, onManualAssign }) {
          ...
          <div>
            ...
            <div className="relative">
              <select value={department} onChange={(e) => setDepartment(e.target.value)}
                ...
                {["Customer Support", "Operations", "Administration"].map(d => <option key={d} value={d}>{d}</option>)}
              </select>
...
export function LeaveRequestRoutingCard({ currentDepartment, currentRoutingStatus, currentRoutingReason, loading, onAutoRoute, onManualAssign }) {
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <div className="relative">
              <select value={department} onChange={(e) => setDepartment(e.target.value)}
                className="w-full appearance-none rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-3 pr-8 text-sm text-slate-900 outline-none transition-all duration-200 focus:border-slate-900 focus:bg-white cursor-pointer">
                {["HR", "Administration", "Operations"].map(d => <option key={d} value={d}>{d}</option>)}
              </select>
              <svg className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
            </div>
          </div>
...
export default ComplaintRoutingCard;