export default function ComplaintSearchFilter({
  searchText,
  statusFilter,
  onSearchChange,
  onStatusChange,
    ...
    <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_12px_40px_rgba(15,23,42,0.05)]">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Quickly find complaint records by keyword and status.</p>
        </div>
        {hasFilters && (
          <button
            type="button"
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          <label className="mb-2 block text-sm font-medium text-slate-700">Search complaints</label>
          <input
            type="text"
            value={searchText}
