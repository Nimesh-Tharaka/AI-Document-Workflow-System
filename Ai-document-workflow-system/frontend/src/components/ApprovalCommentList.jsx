  ...
  return (
    <div className="rounded-[26px] border border-slate-200 bg-white p-6 shadow-[0_10px_35px_rgba(15,23,42,0.05)] transition-all duration-300 hover:shadow-[0_16px_45px_rgba(15,23,42,0.07)]">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold text-slate-900">Approval Comments</h2>
          <p className="mt-1 text-sm text-slate-500">Review trail and feedback history.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {comments.map((item, index) => (
            <div
              key={item.id}
              className="group rounded-2xl border border-slate-200 p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-sm"
            >
              <div className="flex gap-4">
                <div className="mt-1 hidden h-3 w-3 rounded-full bg-slate-300 md:block" />
                <div className="flex-1">
                  ...
                  <p className="leading-7 text-slate-700">{item.comment || "-"}</p>
                </div>
              </div>
            </div>
          ))}
