...
export default function UploadComplaintCard({ onUploadSuccess }) {
  ...
  async function handleSubmit(event) {
    ...
    if (!selectedFile) {
      setError("Please choose a complaint file first.");
      return;
    }
  ...
  return (
    <div className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-[0_12px_40px_rgba(15,23,42,0.05)]">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-semibold text-slate-900">Upload Complaint</h2>
          <p className="mt-1 text-sm text-slate-500">Upload PDF, JPG, JPEG, or PNG complaint files.</p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          <span className="text-base font-semibold text-slate-800">Choose complaint file</span>
        ...
        >
          {loading ? "Processing..." : "Upload Complaint"}
        </button>
